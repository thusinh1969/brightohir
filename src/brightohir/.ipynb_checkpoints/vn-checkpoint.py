...
class _VNRegistry:
    ...
    def load_records(self, key: str, records: list[dict[str, Any]]) -> int:
        """Load code system from a list of dicts (for API/database/programmatic use).

        Each dict must have at least 'code' and 'display_vi' keys.

        Usage:
            # From API response
            data = requests.get("https://api.example.com/icd10").json()
            VN.load_records("icd10", data)

            # From database
            rows = db.execute("SELECT * FROM icd10_vn").fetchall()
            VN.load_records("icd10", [dict(r) for r in rows])

            # Manual
            VN.load_records("icd10", [
        ...
        if not meta:
            raise ValueError(f"Unknown code system key: {key}. "
                             f"Available: {list(VN_CODE_SYSTEMS.keys())}")
    ...
    def _get_system(self, key: str) -> VNCodeSystem:
        ...
        if cs is None:
            ...
            if not self._loaded:
                raise RuntimeError(
                    "VN data not loaded. Options:\n"
                    "  VN.load()                          # bundled sample data\n"
                    "  VN.load('/path/to/data/vn/')       # full JSONL files\n"
                    "  VN.load_records('icd10', [...])     # from API/database\n")
